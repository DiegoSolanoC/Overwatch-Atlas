...
function writeHeroesArchiveLocal(events) {
    try {
        ...
        console.warn('[hero-biography] Could not write heroes archive to localStorage:', err);
    }
...
function postHeroesArchiveToDevApi(events) {
    ...
    try {
        ...
        fetch(apiUrl, {
            ...
            .catch((err) => {
                console.warn('[hero-biography] heroes archive API save failed:', err);
                window.updateAppStatus?.(
        ...
        console.warn('[hero-biography] heroes archive API save skipped:', err);
    }
}
