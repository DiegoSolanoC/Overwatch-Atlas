...
const STRIP_HIDDEN_CLASS = 'hero-biography-hero-filters--hidden';
const HOST_COLLAPSED_CLASS = 'hero-biography-mode--hero-filters-collapsed';
...
export function mountHeroBiographyChipStripToggle() {
    ...
    const btn = createHeaderHubButton({
        id: TOGGLE_ID,
        className: 'dock-globe-rail__btn hero-biography-select-hero-toggle',
    ...
    if (!btn?.isConnected) {
        console.warn('[hero-biography] Select Hero toggle could not mount');
        return;
    }

    if (!dockParent) {
        btn.classList.add('hero-biography-select-hero-toggle--floating');
    }
