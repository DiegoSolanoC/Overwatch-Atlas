...
export function initHeroBiographyPhraseButton(controlsRow) {
    ...
    phraseBtn.className = 'hero-biography-mode__phrase-btn';
    phraseBtn.setAttribute('aria-label', 'Play random phrase');
    phraseBtn.disabled = true;

    const icon = document.createElement('img');
    icon.className = 'hero-biography-mode__phrase-btn-icon';
    icon.src = HERO_BIOGRAPHY_PHRASE_ICON_PATH;
    icon.alt = '';
    icon.decoding = 'async';

    const label = document.createElement('span');
    label.className = 'hero-biography-mode__phrase-btn-label';
...
export async function setHeroBiographyPhraseButtonHero(heroFilterKey) {
    ...
    try {
        ...
        console.warn('[hero-biography] Could not load hero phrases:', err);
        activePhraseFiles = [];
    }
