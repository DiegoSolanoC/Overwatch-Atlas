/**
 * WorldviewInteractionController - Handles mouse/touch controls and marker interactions
...
export class WorldviewInteractionController {
    ...
    setPlanesVisibility(visible) {
        ...
        // Use WorldviewGlobeController's animatePlaneScale for smooth squash/stretch animation
    ...
    restorePlanesVisibility() {
        // Use WorldviewGlobeController's updatePlaneVisibility to check current page and show/hide accordingly
    ...
    onContextMenu(event) {
        ...
        console.log('[WorldviewInteractionController] onContextMenu called');
        
        // Check if right-clicking on a marker to force cycle overlapping markers
        const camera = this.sceneModel.getCamera();
        if (!camera) {
            console.log('[WorldviewInteractionController] No camera');
            return;
        }
        
        const container = document.getElementById('globe-container');
        if (!container) {
            console.log('[WorldviewInteractionController] No container');
            return;
        }
        ...
        console.log('[WorldviewInteractionController] Mouse coords:', mouse.x, mouse.y);
        ...
        console.log('[WorldviewInteractionController] Clickable objects:', clickableObjects.length);
        
        const intersects = raycaster.intersectObjects(clickableObjects);
        console.log('[WorldviewInteractionController] Intersects:', intersects.length);
        
        if (intersects.length > 0) {
            // Right-clicked on a marker - force cycle if it's in an overlap group
            const clickedMarker = intersects[0].object;
            console.log('[WorldviewInteractionController] Clicked marker:', clickedMarker.userData.eventName);
            if (window.globeEventMarkerManager && typeof window.globeEventMarkerManager.forceCycleMarker === 'function') {
                console.log('[WorldviewInteractionController] Calling forceCycleMarker');
                ...
                setTimeout(() => {
                    ...
                    if (group) {
                        ...
                        if (nextMarker && this.markerService) {
                            console.log('[WorldviewInteractionController] Manually updating hover state for:', nextMarker.userData.eventName);
