 ...
 *   2. Constructs the `ComponentOrchestrator` with those pairs.
 ...
 * which is why it pairs with (but is distinct from) `ComponentOrchestrator`:
 *   - `LoadingOrchestrator` (this file)  boot-time wiring + window globals
 *   - `ComponentOrchestrator` (a class)   the runtime that owns lifecycle
 *
...
const componentOrchestrator = new ComponentOrchestrator(loadedComponents, loaders, unloaders);

const runMenuComponents = () => componentOrchestrator.runMenuComponents();
const runUniversalFeatures = (options = {}) => componentOrchestrator.runUniversalFeatures(options);
const runGlobeComponents = (isAutoLoad = false) => componentOrchestrator.runGlobeComponents(isAutoLoad);
const killMenuComponents = () => componentOrchestrator.killMenuComponents();
const killUniversalFeatures = () => componentOrchestrator.killUniversalFeatures();
const restoreMainMenu = () => componentOrchestrator.restoreMainMenu();
const killGlobeComponents = () => componentOrchestrator.killGlobeComponents();
const runGlossaryComponents = (isAutoLoad = false) => componentOrchestrator.runGlossaryComponents(isAutoLoad);
const killGlossaryComponents = () => componentOrchestrator.killGlossaryComponents();
const runBiographyComponents = (isAutoLoad = false) => componentOrchestrator.runBiographyComponents(isAutoLoad);
const killBiographyComponents = () => componentOrchestrator.killBiographyComponents();
