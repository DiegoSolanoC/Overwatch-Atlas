/**
 * removeElement — DOM removal helpers used by loader unload paths.
 *
 * - `removeElementById(id, msg, checkParent?)` — the dominant entry point.
 * - `removeElementBySelector(sel, msg, parent?)` — selector variant.
 * - `removeElementsByIds([{id, message, checkParent?}])` — batch wrapper.
 *
 * Each one calls `.remove()` and (optionally) emits a status-line toast on
 * success.
 */
