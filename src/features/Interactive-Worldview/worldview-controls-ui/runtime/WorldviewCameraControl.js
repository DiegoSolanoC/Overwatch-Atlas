/**
 * WorldviewCameraControl - Handles camera zoom, movement, and reset
 */

class WorldviewCameraControl {
    ...
    resetCameraToDefault(onPlanesVisibilityRestore) {
        console.log('[WorldviewCameraControl.resetCameraToDefault] Called');
        
        // CRITICAL: Stop any station/marsShip following first
        console.log('[WorldviewCameraControl.resetCameraToDefault] Stopping station/marsShip follow');
        ...
        console.log('[WorldviewCameraControl.resetCameraToDefault] isMapView:', isMapView);
        console.log('[WorldviewCameraControl.resetCameraToDefault] camera exists:', !!camera);
        console.log('[WorldviewCameraControl.resetCameraToDefault] globe exists:', !!globe);
        console.log('[WorldviewCameraControl.resetCameraToDefault] camera position before:', camera ? camera.position.clone() : 'no camera');
        console.log('[WorldviewCameraControl.resetCameraToDefault] globe rotation before:', globe ? { x: globe.rotation.x, y: globe.rotation.y, z: globe.rotation.z } : 'no globe');
        
        if (!camera || !globe) {
            console.log('[WorldviewCameraControl.resetCameraToDefault] Early return - missing camera or globe');
            return;
        }

        if (isMapView) {
            console.log('[WorldviewCameraControl.resetCameraToDefault] Map view mode - calling map2dLite.resetView');
            ...
            console.log('[WorldviewCameraControl.resetCameraToDefault] Map view reset complete');
            return;
        }
        
        // Restore plane visibility for Moon/Mars events (they might have been hidden by Earth event zoom)
        if (onPlanesVisibilityRestore) {
            console.log('[WorldviewCameraControl.resetCameraToDefault] Restoring plane visibility');
            onPlanesVisibilityRestore();
        } else {
            // If no callback provided, try to restore via interactionController
            console.log('[WorldviewCameraControl.resetCameraToDefault] No restore callback, trying interactionController');
            window.globeController?.interactionController?.restorePlanesVisibility?.();
        }
        
        // Disable auto-rotate
        console.log('[WorldviewCameraControl.resetCameraToDefault] Disabling auto-rotate');
        this.sceneModel.setAutoRotate(false);
        if (this.sceneModel.autoRotateTimeout) {
            clearTimeout(this.sceneModel.autoRotateTimeout);
            this.sceneModel.autoRotateTimeout = null;
        ...
        console.log('[WorldviewCameraControl.resetCameraToDefault] isMobilePortrait:', isMobilePortrait);
        console.log('[WorldviewCameraControl.resetCameraToDefault] defaultZoom:', defaultZoom);

        // Map view: fit whole map (same perspective 3D pipeline as Moon/Mars planes).
        if (isMapView && camera && earthMapPlane && renderer) {
            console.log('[WorldviewCameraControl.resetCameraToDefault] Calculating map fit zoom');
            const rect = renderer.domElement.getBoundingClientRect();
            const viewportW = Math.max(1, rect.width);
            const viewportH = Math.max(1, rect.height);
            const aspect = viewportW / viewportH;
            ...
            console.log('[WorldviewCameraControl.resetCameraToDefault] Calculated map fit zoom:', defaultZoom);
        }

        const targetPosition = new THREE.Vector3(0, 0, defaultZoom);
        const targetRotation = { x: 0, y: 0, z: 0 };
        console.log('[WorldviewCameraControl.resetCameraToDefault] targetPosition:', targetPosition);
        console.log('[WorldviewCameraControl.resetCameraToDefault] targetRotation:', targetRotation);
        ...
        if (isMapView && earthMapPlane && renderer) {
            console.log('[WorldviewCameraControl.resetCameraToDefault] Map view - snapping to position');
            camera.position.copy(targetPosition);
            const clamped = this._clampMapXY(camera.position.x, camera.position.y, camera.position.z);
            camera.position.x = clamped.x;
            camera.position.y = clamped.y;
            camera.lookAt(camera.position.x, camera.position.y, 0);
            console.log('[WorldviewCameraControl.resetCameraToDefault] Map view snap complete, final position:', camera.position.clone());
            return;
        }
        
        // Animate camera to default position (globe)
        console.log('[WorldviewCameraControl.resetCameraToDefault] Globe view - animating to default');
        const startPosition = camera.position.clone();
        const startRotation = {
            x: globe.rotation.x,
            y: globe.rotation.y,
        ...
        console.log('[WorldviewCameraControl.resetCameraToDefault] startPosition:', startPosition);
        console.log('[WorldviewCameraControl.resetCameraToDefault] startRotation:', startRotation);
        ...
        const animate = () => {
            ...
            if (progress < 1) {
                ...
                console.log('[WorldviewCameraControl.resetCameraToDefault] Animation complete');
                console.log('[WorldviewCameraControl.resetCameraToDefault] Final camera position:', camera.position.clone());
                console.log('[WorldviewCameraControl.resetCameraToDefault] Final globe rotation:', { x: globe.rotation.x, y: globe.rotation.y, z: globe.rotation.z });
            }
        };
...
if (typeof module !== 'undefined' && module.exports) {
    module.exports = WorldviewCameraControl;
}

// Make globally accessible
if (typeof window !== 'undefined') {
    window.WorldviewCameraControl = WorldviewCameraControl;
}
