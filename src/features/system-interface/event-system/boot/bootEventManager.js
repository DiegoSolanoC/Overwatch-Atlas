...
export async function initializeEventManager() {
    ...
    if (typeof window.EventManager === 'undefined' && !existingScript) {
        // Load EventManager as ES module (relative imports inside coordinator/)
        return await new Promise((resolve, reject) => {
            const script = document.createElement('script');
            script.type = 'module';
            script.src = 'src/features/system-interface/coordinator/EventManager.js?' + Date.now(); // Cache busting
            script.onload = async () => {
                try {
...
async function createEventManagerInstance() {
    const ensureGlobalServiceScript = (globalKey, srcPath) =>
        new Promise((resolve, reject) => {
            ...
            if (/event-system\/(data|render|form|listeners|edit|interaction|drag-drop|lookup)\//.test(srcPath)) {
                script.type = 'module';
            }
    ...
    await ensureGlobalServiceScript('EventDataService', 'src/features/system-interface/event-system/data/EventDataService.js');
    await ensureGlobalServiceScript('EventInitService', 'src/features/system-interface/event-system/boot/runEventManagerInit.js');
