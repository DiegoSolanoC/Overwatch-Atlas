/**
 * CityCoordinateLookup — backs the "Lookup" button in the edit form.
 *
 * Direction is **forward**: city name → `(lat, lon)`. The user can pick between two paths
 * via the "Use Code Lookup" checkbox:
 *   - **Code path** (default): scan the in-memory datasets via `findCityCoordinates`.
 *   - **Online path**: query OpenStreetMap Nominatim and accept the first result.
 *
 * On success, the form's lat/lon and `cityDisplayName` fields are filled in and an `alert()`
 * confirms the match. Failure modes fall back gracefully and re-enable the button.
 *
 * (Compare with `LocationLabelResolver`, which does the reverse direction for already-saved
 * events on the manager list.)
 *
 * Exposed as `window.CityCoordinateLookup` (new) and `window.CityLookupService` (legacy alias).
 */
...
if (typeof window !== 'undefined') {
    const instance = new CityCoordinateLookup();
    window.CityCoordinateLookup = instance;
    // Legacy alias — kept until all consumers migrate.
    window.CityLookupService = instance;
}
