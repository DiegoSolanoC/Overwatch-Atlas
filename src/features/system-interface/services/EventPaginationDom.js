/**
 * EventPaginationDom — builds and mounts the event pagination DOM
 * (#eventPagination), positions it inside the persistent #paginationDock
 * below the layout container, and installs the trapezoid border + collapse
 * strip chrome that frames the dock.
 *
 * Pure DOM factory. Click wiring (prev/next page, prev/next event, page
 * input) is owned by `EventNavigationService.js`; the dock collapse behavior
 * is owned by `../utils/PaginationDockCollapse.js`; thumbnail markup comes
 * from `../managers/helpers/PaginationThumbMarkup.js`.
 *
...
export function createEventPagination() {
    ...
    // Buttons may have been moved to dock rails — reuse the existing dock
    // when possible; otherwise clear orphaned nav buttons before rebuilding.
