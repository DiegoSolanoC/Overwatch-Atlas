/**
 * Dump page 25 events (#241–250) + refresh used-commentary.json
 */
import fs from 'fs';
...
const page = events.slice(240, 250);
console.log('\n=== PAGE 25 (#241–250) ===\n');
page.forEach((e, idx) => {
  const n = 241 + idx;
  ...
  if (Array.isArray(e.tags)) {
    for (const t of e.tags) console.log('  [tag]', JSON.stringify(t));
  }
  // print place/hero/npc style metadata if present
  for (const key of ['places', 'heroes', 'npcs', 'factions', 'metadata', 'links']) {
    if (e[key]) console.log(`  [${key}]`, JSON.stringify(e[key]).slice(0, 300));
  }
  if (e.country || e.why) console.log('  meta', e.country, e.why);
  // dump any structured annotations
  if (Array.isArray(e.annotations)) {
    for (const a of e.annotations) console.log('  [ann]', JSON.stringify(a));
  }
  console.log('');
});
